...
require_auth();
require_once __DIR__ . '/../modelos/victimas-use.php';
$q = trim((string)($_GET['q'] ?? ''));
$victimas = $q !== '' ? victimas_find_by_name($q) : victimas_all();
?>
<!doctype html>
<html lang="es">
<head>
    ...
    <div class="card">
      <table class="table">
        ...
        <?php foreach($victimas as $v): ?>
          <tr class="victima-row">
            <td><?php echo (int)$v['id']; ?></td>
            <td><?php echo h(($v['victima_nombre']??'') . ' ' . ($v['victima_apellidos']??'')); ?></td>
            <td><?php echo h(($v['victima_tipo_documento']??'') . ' ' . ($v['victima_num_documento']??'')); ?></td>
            <td><?php echo h($v['victima_edad'] ?? ''); ?></td>
          </tr>
        <?php endforeach; ?>
        </tbody>
      </table>

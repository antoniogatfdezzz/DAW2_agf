/* Juego 2D: versión clásica (script no módulo). Si el archivo no se carga al abrir localmente,
   asegúrate de abrir mediante servidor (python3 -m http.server) o que el navegador permita scripts locales.
*/

document.addEventListener('DOMContentLoaded', function(){
  console.log('[Juego] DOMContentLoaded - inicializando');
  const el = {
  ...
  function gameOver(){
    ...
    if(el.finalScore) el.finalScore.textContent = formatScore(state.score);
    if(el.overlay) el.overlay.classList.remove('hidden');
  }

  // Guardar listeners después de comprobar elementos
  if(el.startBtn) el.startBtn.addEventListener('click', ()=>{ setDifficulty(el.difficulty.value); startGame(); });
  if(el.restartBtn) el.restartBtn.addEventListener('click', ()=>{ setDifficulty(el.difficulty.value); startGame(); });
  if(el.block) el.block.addEventListener('click', onBlockClick);
  if(el.difficulty) el.difficulty.addEventListener('change', ()=>{ setDifficulty(el.difficulty.value); });

  // Inicializar
  if(el.overlay) el.overlay.classList.add('hidden');
  setDifficulty((el.difficulty && el.difficulty.value) || 'normal');
  ...
  window.addEventListener('resize', ()=>{
    ...
    if(left > maxX || top > maxY){ positionBlockRandom(); }
  });
